	...
	<div class="col-lg-6">
		...
		<form action="{{ url('events') }}" method="POST">
			...
			<div class="form-group @if($errors->has('name')) has-error has-feedback @endif">
				...
				<input type="text" class="form-control" name="name" placeholder="Client Name" value="{{ old('name') }}">
				@if ($errors->has('name'))
			...
			<div class="form-group @if($errors->has('title')) has-error has-feedback @endif">
				...
				<input type="text" class="form-control" name="title" placeholder="Booking Title" value="{{ old('title') }}">
				@if ($errors->has('title'))
					<p class="help-block"><span class="glyphicon glyphicon-exclamation-sign"></span> 
					{{ $errors->first('title') }}
					</p>
